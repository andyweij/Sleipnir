...
class TestAgent:
    def test_agent_early_stopping(self):
        ...
        mock_client.chat_completion.return_value = Message(role="assistant", content="I am a mocked response.")
    ...
    def test_agent_tool_calling_loop(self):
        ...
        mock_client.chat_completion.side_effect = [
            ...
            Message(
                role="assistant",
                tool_calls=[ToolCall(id="call_123", name="dummy_weather", arguments='{"location": "Taipei"}')],
            ),
            # 第二次：LLM 看到工具回傳的 Observation 後，給出最終答案
            Message(role="assistant", content="The weather in Taipei is sunny."),
        ]
    ...
    def test_agent_tool_execution_error(self):
        ...
        mock_client.chat_completion.side_effect = [
            ...
            Message(
                role="assistant", tool_calls=[ToolCall(id="call_err", name="dummy_weather", arguments="{Bad JSON]")]
            ),
            # 第二次：Agent 應該要把 Error 訊息回傳給 LLM，然後 LLM 道歉並給出回答
            Message(role="assistant", content="Sorry, I messed up the JSON."),
        ]
