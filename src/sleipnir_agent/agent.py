...
class Agent:
    ...
    def run(self, user_input: str) -> str:
        ...
        messages = [Message(role="system", content=dynamic_system_prompt), Message(role="user", content=user_input)]
        ...
        for iteration in range(self.max_iterations):
            ...
            response_msg = self.client.chat_completion(
                model=self.model, messages=messages, tools=self._tool_schemas if self._tool_schemas else None
            )
            self.trace.append(TraceEvent(kind="llm_response", data={"has_tool_calls": bool(response_msg.tool_calls)}))
            ...
            if response_msg.tool_calls:
                for tool_call in response_msg.tool_calls:
                    self.trace.append(
                        TraceEvent(kind="tool_call", data={"name": tool_call.name, "arguments": tool_call.arguments})
                    )

                    tool_result = self._execute_tool(tool_call.name, tool_call.arguments)

                    self.trace.append(
                        TraceEvent(kind="tool_result", data={"name": tool_call.name, "result": str(tool_result)[:500]})
                    )
                    ...
                    messages.append(
                        Message(role="tool", content=str(tool_result), name=tool_call.name, tool_call_id=tool_call.id)
                    )

                # 迴圈繼續，讓 LLM 看到 Observation 後再做決定
                continue
            else:
                self.trace.append(TraceEvent(kind="final_answer", data={"content": response_msg.content}))
