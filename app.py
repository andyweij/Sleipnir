...
def create_agent(backend_type: str) -> Agent:
    ...
    # 組裝並回傳 Agent 實例
    return Agent(client=client, model=model_name, tools=[web_search_tool], max_iterations=5)
...
async def chat_endpoint(request: ChatRequest):
    try:
        ...
        if request.use_web_search:
            ...
            context_str = "\n".join([f"- [{res.title}]({res.url}): {res.content}" for res in search_results])
...
{context_str}
        ...
        # 回傳給前端
        return {"answer": response_text, "backend_used": request.backend_type}
